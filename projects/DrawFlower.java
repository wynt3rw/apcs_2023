...
import proccessing.core.PApplet;

import controlP5.*;
import javax.swing.*;

public class DrawFlower extends PApplet{
    void drawArchMandala(int numOuterArches, int numInnerArches, float baseOuterRadius, float baseInnerRadius, int numPetals, float distanceFactor, float speed){
        ...
        for(int i = 0; i < numOuterArches; i++){
            ...
            drawArch(baseOuterRadius, baseInnerRadius-i*10, 10, speed, lerpColor(startColor, endColor, amount), 1);
        }
  
        for(int i = 0; i < numInnerArches; i++){
            float amount = map(i + numPetals + numOuterArches, 0, totalArches - 1, 0.0, 1.0);
            drawArch((baseOuterRadius * distanceFactor)+(i*10), baseInnerRadius, 10, speed, lerpColor(startColor, endColor, amount), 1);
        }
    }
}